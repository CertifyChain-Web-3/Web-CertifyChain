...
import { ArrowLeft, Download, Share2, ExternalLink, Shield, FileText, Loader2 } from 'lucide-react';
...
const mockCertificateData = {
  ...
  description: 'This certificate is awarded for successful completion of the Bachelor of Computer Science program with honors, specializing in Artificial Intelligence and Machine Learning.',
  certificateId: '2025-CS-001',
  tokenId: '42',
  transactionHash: '0x8fb1356e2c9c1a9c25d368f1944c1a9cd2bd4d1ac122b0fb8263f3a948826324',
              ...
              <div className="border rounded-lg overflow-hidden w-full max-w-lg">
                ...
                <div className="relative h-80 w-full border-b bg-gray-50 flex items-center justify-center p-6">
                  <div className="text-center border-4 border-blue-800 p-8 w-full h-full flex flex-col items-center justify-center">
                    <div className="text-xl font-serif mb-2 text-blue-800">UNIVERSITY OF TECHNOLOGY</div>
                    <div className="text-sm mb-4">This certifies that</div>
                    <div className="text-xl font-bold mb-1">{certificate.recipientName}</div>
                    <div className="text-sm mb-4">has successfully completed the requirements for the degree of</div>
                    <div className="text-2xl font-bold mb-4">{certificate.title}</div>
                    <div className="text-sm">Awarded on {new Date(certificate.issueDate).toLocaleDateString()}</div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <dl className="space-y-2">
                  <div className="flex flex-col sm:flex-row">
                    <dt className="text-sm font-medium text-gray-500 sm:w-40">Recipient</dt>
                    <dd className="text-sm text-gray-900">{certificate.recipientName}</dd>
                  </div>
                  <div className="flex flex-col sm:flex-row">
                    <dt className="text-sm font-medium text-gray-500 sm:w-40">Issuing University</dt>
                    <dd className="text-sm text-gray-900">{certificate.university}</dd>
                  </div>
                  <div className="flex flex-col sm:flex-row">
                    <dt className="text-sm font-medium text-gray-500 sm:w-40">Issue Date</dt>
                    <dd className="text-sm text-gray-900">{new Date(certificate.issueDate).toLocaleDateString()}</dd>
                  </div>
                  {certificate.expiryDate && (
                    <div className="flex flex-col sm:flex-row">
                      <dt className="text-sm font-medium text-gray-500 sm:w-40">Expiry Date</dt>
                      <dd className="text-sm text-gray-900">{new Date(certificate.expiryDate).toLocaleDateString()}</dd>
                    </div>
                  )}
                  <div className="flex flex-col sm:flex-row">
                    <dt className="text-sm font-medium text-gray-500 sm:w-40">Description</dt>
                    <dd className="text-sm text-gray-900">{certificate.description}</dd>
                  </div>
                </dl>
              </div>
              ...
              <div>
                ...
                <dl className="space-y-2">
                  <div className="flex flex-col">
                    ...
                    <dd className="text-sm text-gray-900 font-mono">{certificate.tokenId}</dd>
                  </div>
                  <div className="flex flex-col">
                    <dt className="text-sm font-medium text-gray-500">University Wallet</dt>
                    <dd className="text-sm text-gray-900 font-mono overflow-hidden text-ellipsis">{certificate.universityWallet}</dd>
                  </div>
                  <div className="flex flex-col">
                    <dt className="text-sm font-medium text-gray-500">Recipient Wallet</dt>
                    <dd className="text-sm text-gray-900 font-mono overflow-hidden text-ellipsis">{certificate.recipientWallet}</dd>
                  </div>
                  <div className="flex flex-col">
                  ...
                  <div className="flex flex-col">
                    ...
                    <dd className="text-sm text-gray-900 font-mono">{certificate.blockNumber}</dd>
                  </div>
                </dl>
              </div>
            </div>
