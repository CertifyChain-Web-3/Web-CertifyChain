...
import { Wallet, ArrowRight, Loader2, CheckCircle, Shield, XCircle } from 'lucide-react';
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (!pageRef.current || !cardRef.current || !titleRef.current || !contentRef.current) {
  ...
  const { login, isAuthenticated, isLoading: isAuthLoading, userRole } = useUser();
  const { checkAddressRole, isLoading: isVerificationLoading } = useAddressVerification();
  const [isChecking, setIsChecking] = useState(false);
  const [loginStep, setLoginStep] = useState<'initial' | 'checking' | 'success' | 'notRegistered'>('initial');
  ...
  useEffect(() => {
    ...
    const forceElementsVisibility = () => {
      ...
      const successElements = document.querySelectorAll('.success-icon, .success-text, .progress-bar');
      ...
      const notRegisteredElements = document.querySelectorAll('.not-registered-icon, .not-registered-text, .progress-bar-blue');
    ...
    if (loginStep === 'success') {
      ...
      gsap.fromTo('.not-registered-icon', 
        { scale: 0, opacity: 0 },
        { 
          scale: 1, 
          opacity: 1, 
      ...
      gsap.fromTo('.not-registered-text', 
        { y: 20, opacity: 0 },
        { 
          y: 0, 
          opacity: 1, 
  ...
  const renderLoginContent = () => {
    if (loginStep === 'success') {
      return (
        <div className="text-center p-6 rounded-lg border border-green-100 dark:border-green-900 bg-green-50 dark:bg-green-900/20">
          <CheckCircle className="success-icon w-16 h-16 text-green-500 dark:text-green-400 mx-auto mb-4" />
          <h2 className="success-text text-xl font-bold mb-2 text-green-800 dark:text-green-300">Login Successful!</h2>
          <p className="success-text text-green-700 dark:text-green-400 mb-4">Redirecting to dashboard...</p>
    ...
    if (loginStep === 'notRegistered') {
      return (
        <div className="text-center p-6 rounded-lg border border-blue-100 dark:border-blue-900 bg-blue-50 dark:bg-blue-900/20">
          <XCircle className="not-registered-icon w-16 h-16 text-blue-500 dark:text-blue-400 mx-auto mb-4" />
          <h2 className="not-registered-text text-xl font-bold mb-2 text-blue-800 dark:text-blue-300">Address Not Registered</h2>
          <p className="not-registered-text text-blue-700 dark:text-blue-400 mb-4">Redirecting to registration...</p>
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
    ...
    return (
      <div className="space-y-4">
        ...
        <div style={{ display: 'block', opacity: 1, visibility: 'visible', minHeight: '44px' }}>
          {isConnected ? (
            <button
              ref={buttonRef}
              onClick={handleLogin}
  ...
  return (
    <div ref={pageRef} className="flex flex-col items-center justify-center min-h-screen p-8 bg-gradient-to-b from-gray-100 to-gray-200 dark:from-gray-900 dark:to-black">
      <div ref={cardRef} className="w-full max-w-md mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 border border-gray-200 dark:border-gray-700">
        <h1 ref={titleRef} className="text-3xl font-bold mb-6 text-center flex items-center justify-center gap-2 text-black dark:text-white">
